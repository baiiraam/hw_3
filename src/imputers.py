...
#         X_out = np.array(X, copy=True, dtype=np.float64) if isinstance(X, np.ndarray) else X.values.copy()
...
# class KNNImputer:
#     """
#     Fast NaN-aware Vectorized KNN Imputer.
#     """
...
#         # Global backup mean if a row has absolutely zero present feature intersections
...
#             # Vectorized computation of NaN-aware euclidean distance to all reference lines
...
#             # Avoid division by zero when rows have absolutely no common elements
...
#                 # Fallback to feature means if no overlap exists anywhere
#                 X_out[i, missing_mask] = self.global_means_[missing_mask]
#                 continue

#             # Vectorized partition is much faster than complete sort
...
#             # Fill missing entries with neighbor mean value or fallback to global mean
