...
#     """Generate polynomial and interaction features with clean combinatoric checks."""
...
#                 if self.interaction_only and len(combos) > 1 and len(set(combos)) != len(combos):
#                     continue
...
#     # ✅ ADDED: get_feature_names_out method
...
#     def __init__(self, n_bins: int = 5, strategy: str = 'uniform', encode: str = 'onehot-dense'):
#         self.n_bins = n_bins
...
#     def fit_transform(self, X: Union[np.ndarray, pd.Series, pd.DataFrame]) -> np.ndarray:
#         return self.fit(X).transform(X)

#     # ✅ ADDED: get_bin_edges method
