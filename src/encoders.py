...
#     """
#     def __init__(self, drop: str = 'first'):
#         self.drop = drop
...
#             cats_to_encode = categories[1:] if (self.drop == 'first' and len(categories) > 0) else categories

#             if not cats_to_encode:
#                 continue

#             # Fixed: Proper vectorized one-hot encoding
#             col_data = X[:, col_idx]
...
#         names = []
#         for col_idx, categories in enumerate(self.categories_):
#             cats = categories[1:] if (self.drop == 'first' and len(categories) > 0) else categories
#             for cat in cats:
...
#     fit_transform to cleanly prevent out-of-fold data leakage.
#     """
#     def __init__(self, smooth: float = 1.0, cv: int = 5):  # ✅ Changed m → smooth
#         self.smooth = smooth
#         self.cv = cv
...
#     def _compute_smoothed_values(self, X: np.ndarray, y: np.ndarray, global_mean: float) -> Dict[Any, float]:
#         df = pd.DataFrame({'X': X, 'y': y})
#         stats = df.groupby('X')['y'].agg(['count', 'mean'])

#         # Apply smoothing: (n * mean + smooth * global_mean) / (n + smooth)
#         smoothed = (stats['count'] * stats['mean'] + self.smooth * global_mean) / (stats['count'] + self.smooth)
...
#         X_arr = self._to_1d_array(X)
#         encoded = pd.Series(X_arr).map(self.encoding_map_).fillna(self.global_mean_).values
#         return encoded.reshape(-1, 1)

#     def fit_transform(self, X: Union[np.ndarray, pd.Series], y: Union[np.ndarray, pd.Series]) -> np.ndarray:
#         X_arr = self._to_1d_array(X)
#         y_arr = self._to_1d_array(y)
...
#             encoded_train[val_idx] = pd.Series(X_val).map(fold_map).fillna(fold_global_mean).values
